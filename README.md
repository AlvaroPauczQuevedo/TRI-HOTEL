...
Este guia explica como instalar e executar o script Desligar.ps1 em qualquer computador Windows.

### 10.1 Pré-requisitos

Antes de executar o script, verifique se o ambiente atende aos seguintes requisitos:

| Requisito | Versão Mínima | Como Verificar |
|-----------|---------------|-----------------|
| Sistema Operacional | Windows 7 ou superior | `winver` no executar |
| PowerShell | 5.1 ou superior | `$PSVersionTable.PSVersion` |
| .NET Framework | 3.5 ou superior | Painel de Controle > Programas |
| Permissão de Administrador | Sim | Necessário para desligar |
...
1. **Crie uma pasta** para armazenar os arquivos:
   ```
   C:\Programas\Desligar
   ```

2. **Copie os arquivos** para a pasta criada:
   - `Desligar.ps1` - Script principal
   - `README.md` - Documentação (opcional)

#### Passo 2: Configurar Política de Execução

Por padrão, o Windows bloqueia a execução de scripts para segurança. Execute:
...
| Política | Descrição |
|----------|-----------|
| `Restricted` | Bloqueia todos os scripts (padrão) |
| `AllSigned` | Permite apenas scripts assinados |
| `RemoteSigned` | Permite scripts locais e remotos assinados |
...
### 10.3 Como Executar o Script

#### Método 1: Via PowerShell (Recomendado)

1. Abra o **PowerShell** (não precisa ser como administrador para visualizar)
2. Navegue até a pasta do script:
   ```powershell
   cd "C:\Programas\Desligar"
   ```
3. Execute o script:
   ```powershell
   .\Desligar.ps1
...
3. Execute:
   ```
   powershell -ExecutionPolicy Bypass -File Desligar.ps1
   ```
...
3. Digite:
   ```
   powershell -ExecutionPolicy Bypass -File "C:\Programas\Desligar\Desligar.ps1"
   ```
4. Clique em **Avançar** e defina um nome (ex: "Desligar PC")
5. Clique em **Concluir**

---

### 10.4 Como Usar a Interface

Ao executar o script, uma janela será exibida:
...
| Ação | Resultado |
|------|-----------|
| **Clicar em "Sim"** | Desliga o computador imediatamente |
| **Clicar em "Cancelar"** | Fecha a janela, nada acontece |
| **Esperar timer chegar a zero** | Desliga automaticamente |
| **Pressionar Escape** | Cancela e fecha a janela |

---

### 10.5 Configurações Opcionais

#### Alterar Tempo do Timer

Edite a linha 4 do arquivo `Desligar.ps1`:

```powershell
$tempoTotal = 120  # segundos (padrão: 120 = 2 minutos)
```
...
#### Criar Atalho com Timer Personalizado

Para criar um atalho com tempo diferente, use:

```
powershell -ExecutionPolicy Bypass -Command "& { $tempoTotal = 60; & 'C:\Programas\Desligar\Desligar.ps1' }"
```
...
Para remover o script do computador:

1. **Delete a pasta** `C:\Programas\Desligar`
2. **Remova atalhos** criados na área de trabalho
3. (Opcional) **Restaure a política de execução** original:
   ```powershell
...
### 10.7 Solução de Problemas Comuns

| Problema | Causa | Solução |
|----------|-------|---------|
| "Execução de scripts bloqueada" | Política restritiva | Execute `Set-ExecutionPolicy RemoteSigned` |
| "Acesso negado" ao desligar | Sem permissão admin | Execute como Administrador |
| "Janela não aparece" | .NET ausente | Instale .NET Framework 3.5+ |
| "Caractere ⚠ não aparece" | Fonte emoji ausente | Use outro ícone ou imagem |
| "Timer não conta" | Erro no timer | Verifique se `$timer.Start()` está presente |

---

### 10.8 Executar em Rede (Domínio)

Em ambientes corporativos com domínio:

1. **Coloque o script** em uma pasta compartilhada
2. **Crie um arquivo .bat** para facilitar:
   ```batch
   @echo off
   powershell -ExecutionPolicy Bypass -File "\\servidor\scripts\Desligar.ps1"
   ```
3. **Distribua** via GPO ou software de deployment
